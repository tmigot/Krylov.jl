...
"""
function craig_bis(A, b :: AbstractVector{T}; kwargs...) where T <: AbstractFloat
  solver = CraigSolver(A, b)
  craig_bis!(solver, A, b; kwargs...)
end

function craig_bis!(solver :: CraigSolver{T,S}, A, b :: AbstractVector{T};
                M=I, N=I, sqd :: Bool=false, λ :: T=zero(T), atol :: T=√eps(T),
                btol :: T=√eps(T), rtol :: T=√eps(T), conlim :: T=1/√eps(T), itmax :: Int=0,
